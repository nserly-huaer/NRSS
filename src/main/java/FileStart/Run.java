...
public class Run {
    ...
    public boolean d(OutputStream out, InputStream in) throws IOException {
        boolean dd = false;
        Logger logger = LogManager.getLogger(Run.class);
        String SP1;
        out.write("-----------软件七合一(软件所有权归nserly所有，未经允许，禁止复制、摘用!)-----------".getBytes());
...
//        logger.info("输入$Exit退出");
        ...
        out.write("请输入你要运行的软件(1.数学工具;2.文件管理;3.猜数字（游戏）)".getBytes());
...
//            System.out.println(version);
        ...
        if (!dd) {
            out.write("运行中触发异常，请与开发者联系！".getBytes());
            return false;
        }
        return true;
    }
